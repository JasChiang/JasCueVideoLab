...
9:16 的 crop 決策必須可回放與診斷。除 target、Grounding、SAM track 與 fallback 外，renderer 需保存每個 crop keyframe 的片段相對時間、追蹤中心、平滑後中心及實際 `crop_x_pixels`。`primary_center` 表示次要 context 可以犧牲，不表示 primary target 可以被裁掉；主要 target 比 9:16 視窗寬時應換候選或 fail closed。
