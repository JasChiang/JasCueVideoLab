...
全庫媒體登錄／shot detection
  ...
  → 每個 shot 的少量代表幀（coarse visual catalog）
  ...
  → Gemini image Grounding 選對語意實例
  → SAM 2.1 限制在單一 shot 內傳播 mask
...
- 所有後續產物使用 source hash、prompt hash、schema hash、model ID 與 frame hash 組成 cache key。
...
- 每個 shot 至少保存開始、中間、結束與一張視覺變化較高的代表幀，不只依賴全片等距 2 秒。
...
- SAM 2.1 將 seed 精煉成 mask，只在同一 shot 向前／向後傳播。
...
Repository 已有全庫 2 秒 catalog、immutable frame-ID selection、FFmpeg shot PTS、exact-frame Grounding、SAM 2.1 shot-local propagation、雙比例 review cut 與本機 blind Grounding review app。`storyboard-temporal` 可手動把全片間隔降到 250 ms（4 FPS）。

尚未完成 Full v1 的 per-clip full-video Clip Card、`MM:SS` coarse Event Map、本機衍生時間、候選區間自動 dense refinement、8 FPS 局部 frame IDs，以及把 coarse/dense review 整合進同一個 UI。本文件描述目標架構與驗收標準，不應被引用為已完成功能清單。
