...
from .full_v1 import create_dense_event_catalog, mmss_to_ms
...
def _render_review(
    *,
    ...
    pages = "".join(
        f'<img src="{html.escape(Path(path).as_uri())}" alt="dense contact sheet">'
        for path in catalog.contact_sheet_paths
    )
    ...
    start_seconds = (decision.handle_in_ms or catalog.source_start_ms) / 1000
    preview = (
